<script lang="ts">
	...
	import { Plane, Clock, DollarSign, Pencil, Trash2, Hash, ArrowRight } from '@lucide/svelte';
	...
	async function handleDeleteFlight() {
		try {
			await deleteFlight({ flightId: flight.id }).updates(getItinerary(itineraryId));
		} finally {
		}
	}
</script>
...
<div class="card flex items-start justify-between p-4 shadow-sm">
	<div class="flex gap-3">
		...
		<div>
			<p class="text-xs font-semibold tracking-wide text-sky-600 uppercase">Flight</p>
...
<EditFlightDialog {flight} {itineraryId} bind:open={editDialogOpen} showTrigger={false} />
<DeleteDialog
	label="flight"
