...
<div class="p-4">
	<div class="mb-4">
		...
		<p class="text-sm text-muted-foreground">{days.length} {days.length === 1 ? 'day' : 'days'} planned · click a day to edit</p>
	</div>
	...
	<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
		{#each days as day (day.id)}
			...
			<button
				...
				class="group rounded-xl border bg-card p-3 text-left shadow-sm transition-all hover:border-primary hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/40"
			>
				...
				<p class="truncate font-serif text-sm font-medium leading-snug">{day.location}</p>
				...
				<div class="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
					...
					{/if}
					{#if total > 0}
						<span class="flex items-center gap-0.5 ml-auto font-medium text-foreground">
